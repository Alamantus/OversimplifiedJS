...
var leftMouse, middleMouse, rightMouse;
leftMouse = isInternetExplorer() ? 1 : 0;
middleMouse = isInternetExplorer() ? 4 : 1;
rightMouse = 2;
var mouse = {
	x: 0,
	y: 0,
...
Controls.Add = function(name, keycode) {
...
Room.prototype.Draw = function () {
	...
	if (this.background.loaded) {
		context.drawImage(self.background, camera.x, camera.y, camera.width, camera.height, 0, 0, self.background.width * ((camera.firstWidth / self.background.width) * camera.xScale), self.background.height * ((camera.firstHeight / self.background.height) * camera.yScale));
	}
...
GameObject.prototype.Draw = function () {
	...
	if (self.image.animations[animation]) {
		...
		var width = self.image.animations[animation].width * self.image.xScale * camera.xScale;
		var height = self.image.animations[animation].height * self.image.yScale * camera.yScale;
		...
		if (isOnCamera(self)) {
			...
			context.translate((self.x - camera.x) * camera.xScale, (self.y - camera.y) * camera.yScale);
...
GameObject.prototype.MoveTo = function (x, y, speed) {
	...
	if (this.x < x) {
		this.x += speed * camera.xScale;
	}
	if (this.x > x) {
		this.x -= speed * camera.xScale;
	}
	if (this.y < y) {
		this.y += speed * camera.yScale;
	}
	if (this.y > y) {
		this.y -= speed * camera.yScale;
	}
	...
	if (checkCollisions) {
		for (var vert = 0; vert < this.yBound * 2; vert++) {
			...
			if (!collisionLeft) {	//If this has already been flagged true, don't make it false again.
				collisionLeft = xSpeed < 0 && CollisionAtPoint((this.x - this.xBound) + (xSpeed * camera.xScale), yToCheck);
			}
			if (!collisionRight) {	//If this has already been flagged true, don't make it false again.
				collisionRight = xSpeed > 0 && CollisionAtPoint((this.x + this.xBound) + (xSpeed * camera.xScale), yToCheck);
			}
		}
		for (var hor = 0; hor < this.xBound * 2; hor++) {
			...
			if (!collisionUp) {		//If this has already been flagged true, don't make it false again.
				collisionUp = ySpeed < 0 && CollisionAtPoint(xToCheck, (this.y - this.yBound) + (ySpeed * camera.yScale));
			}
			if (!collisionDown) {	//If this has already been flagged true, don't make it false again.
				collisionDown = ySpeed > 0 && CollisionAtPoint(xToCheck, (this.y + this.yBound) + (ySpeed * camera.yScale));
			}
		}
	}
	if (!checkCollisions || (!collisionLeft && !collisionRight && !collisionUp && !collisionDown)) {
		this.x += xSpeed * camera.xScale;
		this.y += ySpeed * camera.yScale;
	}
}
...
var DEBUG = {
	DrawBoundingBox: function (object) {
		...
		context.fillRect((object.x - object.xBound - camera.x) * camera.xScale, (object.y - object.yBound - camera.y) * camera.yScale, object.xBound * 2, object.yBound * 2);
...
function SetupCamera () {
	...
	camera.firstWidth = camera.width;
	camera.firstHeight = camera.height;
}
...
function Draw () {
	...
	camera.xScale = Math.clamp(camera.xScale, camera.minScale, camera.maxScale);
	camera.yScale = Math.clamp(camera.yScale, camera.minScale, camera.maxScale);
	camera.width = camera.firstWidth / camera.xScale;
	camera.height = camera.firstHeight / camera.yScale;
